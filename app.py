...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:ashwin@localhost/postgres'
...
def signup():
    ...
    if form.validate_on_submit():
        ...
        if domain == "student.edu.ac.in":
            role = 2
        elif domain == "edu.ac.in":
            role = 1
...
def form(faculty_id):
    ...
    if feedbackforms.validate_on_submit():
        ...
        db.session.commit()
        flash("Your Feedback has been recorded! We will help you out!!", category="success")
        return redirect(url_for("dashboard"))
